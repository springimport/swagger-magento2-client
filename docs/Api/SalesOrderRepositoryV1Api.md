...
> \SpringImport\Swagger\Magento2\Client\Model\SalesDataOrderSearchResultInterface salesOrderRepositoryV1GetListGet($searchCriteria)
...
try {
    $result = $apiInstance->salesOrderRepositoryV1GetListGet($searchCriteria);
    print_r($result);
} catch (Exception $e) {
