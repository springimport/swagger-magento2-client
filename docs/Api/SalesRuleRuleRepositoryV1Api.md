...
> \SpringImport\Swagger\Magento2\Client\Model\SalesRuleDataRuleSearchResultInterface salesRuleRuleRepositoryV1GetListGet($searchCriteria)
...
try {
    $result = $apiInstance->salesRuleRuleRepositoryV1GetListGet($searchCriteria);
    print_r($result);
} catch (Exception $e) {
