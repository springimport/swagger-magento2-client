...
> \SpringImport\Swagger\Magento2\Client\Model\EavDataAttributeSetSearchResultsInterface catalogAttributeSetRepositoryV1GetListGet($searchCriteria)
...
try {
    $result = $apiInstance->catalogAttributeSetRepositoryV1GetListGet($searchCriteria);
    print_r($result);
} catch (Exception $e) {
