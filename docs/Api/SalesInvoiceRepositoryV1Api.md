...
> \SpringImport\Swagger\Magento2\Client\Model\SalesDataInvoiceSearchResultInterface salesInvoiceRepositoryV1GetListGet($searchCriteria)
...
try {
    $result = $apiInstance->salesInvoiceRepositoryV1GetListGet($searchCriteria);
    print_r($result);
} catch (Exception $e) {
