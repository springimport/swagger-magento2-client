...
> \SpringImport\Swagger\Magento2\Client\Model\CompanyDataCompanySearchResultsInterface companyCompanyRepositoryV1GetListGet($searchCriteria)
...
try {
    $result = $apiInstance->companyCompanyRepositoryV1GetListGet($searchCriteria);
    print_r($result);
} catch (Exception $e) {
