...
> \SpringImport\Swagger\Magento2\Client\Model\CatalogDataProductSearchResultsInterface catalogProductRepositoryV1GetListGet($searchCriteria)
...
try {
    $result = $apiInstance->catalogProductRepositoryV1GetListGet($searchCriteria);
    print_r($result);
} catch (Exception $e) {
