...
> \SpringImport\Swagger\Magento2\Client\Model\SharedCatalogDataSearchResultsInterface sharedCatalogSharedCatalogRepositoryV1GetListGet($searchCriteria)
...
try {
    $result = $apiInstance->sharedCatalogSharedCatalogRepositoryV1GetListGet($searchCriteria);
    print_r($result);
} catch (Exception $e) {
