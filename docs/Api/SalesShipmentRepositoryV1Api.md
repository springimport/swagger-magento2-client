...
> \SpringImport\Swagger\Magento2\Client\Model\SalesDataShipmentSearchResultInterface salesShipmentRepositoryV1GetListGet($searchCriteria)
...
try {
    $result = $apiInstance->salesShipmentRepositoryV1GetListGet($searchCriteria);
    print_r($result);
} catch (Exception $e) {
