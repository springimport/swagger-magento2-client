...
> \SpringImport\Swagger\Magento2\Client\Model\CustomerDataGroupSearchResultsInterface customerGroupRepositoryV1GetListGet($searchCriteria)
...
try {
    $result = $apiInstance->customerGroupRepositoryV1GetListGet($searchCriteria);
    print_r($result);
} catch (Exception $e) {
