...
> \SpringImport\Swagger\Magento2\Client\Model\CompanyDataRoleSearchResultsInterface companyRoleRepositoryV1GetListGet($searchCriteria)
...
try {
    $result = $apiInstance->companyRoleRepositoryV1GetListGet($searchCriteria);
    print_r($result);
} catch (Exception $e) {
