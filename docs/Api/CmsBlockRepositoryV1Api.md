...
> \SpringImport\Swagger\Magento2\Client\Model\CmsDataBlockSearchResultsInterface cmsBlockRepositoryV1GetListGet($searchCriteria)
...
try {
    $result = $apiInstance->cmsBlockRepositoryV1GetListGet($searchCriteria);
    print_r($result);
} catch (Exception $e) {
