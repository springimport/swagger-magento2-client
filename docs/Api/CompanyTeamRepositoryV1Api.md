...
> \SpringImport\Swagger\Magento2\Client\Model\CompanyDataTeamSearchResultsInterface companyTeamRepositoryV1GetListGet($searchCriteria)
...
try {
    $result = $apiInstance->companyTeamRepositoryV1GetListGet($searchCriteria);
    print_r($result);
} catch (Exception $e) {
